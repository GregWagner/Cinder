...
const uint32_t CONNECTIONS_TOTAL =
    (POINTS_X - 1) * POINTS_Y + (POINTS_Y - 1) * POINTS_X;
...
ClothSimulationApp::ClothSimulationApp()
    : mIterationsPerFrame(16), mIterationIndex(0), mDrawPoints(true),
      mDrawLines(true), mCurrentCamRotation(0.0f), mUpdate(true),
...
void ClothSimulationApp::setupBuffers() {
  ...
  for (j = 0; j < POINTS_Y; j++) {
    ...
    for (i = 0; i < POINTS_X; i++) {
      ...
      positions[n] =
          vec4((fi - 0.5f) * (float)POINTS_X, (fj - 0.5f) * (float)POINTS_Y,
      ...
      if (j != (POINTS_Y - 1)) {
        if (i != 0)
          connections[n][0] = n - 1; // left
        if (j != 0)
          connections[n][1] = n - POINTS_X; // above
        if (i != (POINTS_X - 1))
          connections[n][2] = n + 1; // right
        if (j != (POINTS_Y - 1))
          connections[n][3] = n + POINTS_X; // below
      }
      n++;
    }
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    gl::ScopedVao scopeVao(mVaos[i]);
    {
      ...
      mPositions[i] =
          gl::Vbo::create(GL_ARRAY_BUFFER, positions.size() * sizeof(vec4),
                          positions.data(), GL_STATIC_DRAW);
      {
        // bind and explain the vbo to your vao so that it knows how to
        // distribute vertices to your shaders.
        gl::ScopedBuffer sccopeBuffer(mPositions[i]);
        gl::vertexAttribPointer(POSITION_INDEX, 4, GL_FLOAT, GL_FALSE, 0,
                                (const GLvoid *)0);
        gl::enableVertexAttribArray(POSITION_INDEX);
      }
      // buffer the velocities
      mVelocities[i] =
          gl::Vbo::create(GL_ARRAY_BUFFER, velocities.size() * sizeof(vec3),
                          velocities.data(), GL_STATIC_DRAW);
      {
        // bind and explain the vbo to your vao so that it knows how to
        // distribute vertices to your shaders.
        gl::ScopedBuffer scopeBuffer(mVelocities[i]);
        gl::vertexAttribPointer(VELOCITY_INDEX, 3, GL_FLOAT, GL_FALSE, 0,
                                (const GLvoid *)0);
        gl::enableVertexAttribArray(VELOCITY_INDEX);
      }
      // buffer the connections
      mConnections[i] =
          gl::Vbo::create(GL_ARRAY_BUFFER, connections.size() * sizeof(ivec4),
                          connections.data(), GL_STATIC_DRAW);
      {
        // bind and explain the vbo to your vao so that it knows how to
        // distribute vertices to your shaders.
        gl::ScopedBuffer scopeBuffer(mConnections[i]);
        gl::vertexAttribIPointer(CONNECTION_INDEX, 4, GL_INT, 0,
                                 (const GLvoid *)0);
  ...
  mLineIndices =
      gl::Vbo::create(GL_ELEMENT_ARRAY_BUFFER, lines * 2 * sizeof(int), nullptr,
                      GL_STATIC_DRAW);
...
void ClothSimulationApp::update() {
  ...
  for (auto i = mIterationsPerFrame; i != 0; --i) {
    ...
    gl::ScopedTextureBind scopeTex(
        mPositionBufTexs[mIterationIndex & 1]->getTarget(),
        mPositionBufTexs[mIterationIndex & 1]->getId());
    ...
    gl::bindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, POSITION_INDEX,
                       mPositions[mIterationIndex & 1]);
    gl::bindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, VELOCITY_INDEX,
                       mVelocities[mIterationIndex & 1]);
...
void ClothSimulationApp::updateRayPosition(const ci::ivec2 &mousePos,
                                           bool useDistance) {
...
void ClothSimulationApp::updateParams() {
  ...
  if (ImGui::DragFloat("Spring Constant", &springConstant, 0.01f, 0.1f,
                       17.0f)) {
